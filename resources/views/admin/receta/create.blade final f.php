...
$(document).ready(function() {
    ...
    $('#btnGuardarPaciente').on('click', function() {
        ...
        if (xhr.responseJSON && xhr.responseJSON.errors) {
            ...
            // Itera sobre los errores y construye el mensaje de error
            $.each(xhr.responseJSON.errors, function(key, value) {
                $('#error-' + key).text(value[0]); // Mostrar el mensaje de error bajo el campo correspondiente
          });
        } else {
            // Si no hay errores específicos de validación en la respuesta del servidor, muestra un mensaje genérico de error
            alert('Error al procesar la solicitud. Inténtelo de nuevo más tarde.');
        }
    }
        });
    });
